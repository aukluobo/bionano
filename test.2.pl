...
foreach my $a_sort(keys %anchor){
	...
	my %tmp_use;
	my @head_path=Find_Path(0,\%anchor,\%sp_mer,$a_sort,$b_sort[0],\%tmp_use);
	my @tail_path=Find_Path(-1,\%anchor,\%sp_mer,$a_sort,$b_sort[-1],\%tmp_use);
	my $len_head=@head_path;my $len_tail=@tail_path;
	my %last_sort;
	my $srt=1;
	for(my $i=$len_head-1;$i>=0;$i--){
		$last_sort{$srt}=$head_path[$i];
		$srt+=1;
	}
	for(my $i=0;$i<$len_tail;$i++){
	...
	foreach my $srt(sort {$a<=>$b} keys %last_sort){
		my @b_sort=sort {$a<=>$b} keys %{$anchor{$srt}};
		#my $start=$b_sort[0];my $end=$b_sort[-1];
		
		foreach my $b_sort(@b_sort){
			my $th=$anchor{$srt}{$b_sort}[3];
			my $spp=$anchor{$srt}{$b_sort}[0];
...
sub Find_Path
{
	my ($type,$hash,$mer_hash,$asort,$bsort,$use)=@_;
	...
	if($mer_hash->{$pre_sp}){
		...
		my $nextb;
		if($nextbsort == $next[0]){$nextb=$next[-1];}else{$nextb=$next[0];}
		if($use->{$nextasort}){
			;
		}else{
			@back=Find_Path($type,$hash,$mer_hash,$nextasort,$nextb,$use);
		}
	}
	push @back,$asort;
